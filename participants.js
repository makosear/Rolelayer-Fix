(function(DR, document) {
	...
	Object.defineProperties(RoleHandler.prototype, {
				...
				return DR.fetch("GET", url)
					.then(
						function(data) {
							...
							DR.FLAIRS[role.code] = data.flair;
							DR.SPRITES[role.code] = data.busts || data.sprites;

							// Now for their flair
							if (("" + data.flair).indexOf("http") === 0) {
		...
		exists: {
			writable: false,
			value: function(user) {
				if (typeof user == "string") return user.toLowerCase() in this.characters;
	...
	function identifyCharacter(line) {
		...
		if (RegExp("\\W(yasuke|matsuda)\\W", "i").test(line)) return DR.ROLES.YASUKE;

		if (RegExp("\\W(ryou?ko|otonashi)\\W", "i").test(line)) return DR.ROLES.RYOUKO;
		...
		if (RegExp("\\W(jataro|kemuri)\\W", "i").test(line)) return DR.ROLES.JATAROU;

		if (RegExp("\\W(kotoko|utsugi)\\W", "i").test(line)) return DR.ROLES.KOTOKO;

		if (RegExp("\\W(nagisa|shingetsu)\\W", "i").test(line)) return DR.ROLES.NAGISA;

		if (RegExp("\\Wmona[ck]a\\W", "i").test(line)) return DR.ROLES.MONACA;

		if (RegExp("\\W(haiji|towa)\\W", "i").test(line)) return DR.ROLES.HAIJI;

		if (RegExp("\\Wtaichi.+fujisaki\\W", "i").test(line)) return DR.ROLES.TAICHI;
		...
		if (RegExp("\\Wmecha[-\\s]?maru\\W", "i").test(line)) return DR.ROLES.MECHAMARU;

		if (RegExp("\\W(nidai|nekomaru)\\W", "i").test(line)) return DR.ROLES.NEKOMARU;

		if (RegExp("\\W(impost[eo]r|twogami)\\W", "i").test(line)) return DR.ROLES.IMPOSTER;

		if (RegExp("\\W(ikusaba|mukuro)\\W", "i").test(line)) return DR.ROLES.MUKURO;

		if (RegExp("\\W(aoi|asahina)\\W", "i").test(line)) return DR.ROLES.AOI;

		if (RegExp("\\W(byakuya|togami)\\W", "i").test(line)) return DR.ROLES.BYAKUYA;

		if (RegExp("\\W(celes(te|tia)?|ludenberg)\\W", "i").test(line)) return DR.ROLES.CELES;

		if (RegExp("\\W(fujisaki|chihiro)\\W", "i").test(line)) return DR.ROLES.CHIHIRO;

		if (RegExp("\\W(hagakure|yasuhiro)\\W", "i").test(line)) return DR.ROLES.YASUHIRO;

		if (RegExp("\\W(hifumi|yamada)\\W", "i").test(line)) return DR.ROLES.HIFUMI;

		if (RegExp("\\W(ishimaru|kiyotaka)\\W", "i").test(line)) return DR.ROLES.KIYOTAKA;

		if (RegExp("\\W(junko|enoshima)\\W", "i").test(line)) return DR.ROLES.JUNKO;

		if (RegExp("\\W(kyou?ko|kirigiri)\\W", "i").test(line)) return DR.ROLES.KYOUKO;

		if (RegExp("\\W(leon|kuwata)\\W", "i").test(line)) return DR.ROLES.LEON;

		if (RegExp("\\W(maizono|sayaka)\\W", "i").test(line)) return DR.ROLES.SAYAKA;

		if (RegExp("\\W(mondo|o[oh]?wada)\\W", "i").test(line)) return DR.ROLES.MONDO;

		if (RegExp("\\Wkomaru\\W", "i").test(line)) return DR.ROLES.KOMARU;

		if (RegExp("\\Wmakoto\\W", "i").test(line)) return DR.ROLES.MAKOTO;

		if (RegExp("\\W(sakura|o[oh]?gami)\\W", "i").test(line)) return DR.ROLES.SAKURA;

		if (RegExp("\\W(tou?ko|fukawa|genocider)\\W", "i").test(line)) return DR.ROLES.TOUKO;

		if (RegExp("\\W(akane|owari)\\W", "i").test(line)) return DR.ROLES.AKANE;

		if (RegExp("\\W(gundh?am|tanaka)\\W", "i").test(line)) return DR.ROLES.GUNDHAM;

		if (RegExp("\\W(hanamura|teruteru)\\W", "i").test(line)) return DR.ROLES.TERUTERU;

		if (RegExp("\\W(hinata|hajime)\\W", "i").test(line)) return DR.ROLES.HAJIME;

		if (RegExp("\\W(koizumi|mahiru)\\W", "i").test(line)) return DR.ROLES.MAHIRU;

		if (RegExp("\\W(natsumi)\\W", "i").test(line)) return DR.ROLES.NATSUMI;

		if (RegExp("\\W(kuzuryu|fuyuhiko)\\W", "i").test(line)) return DR.ROLES.FUYUHIKO;

		if (RegExp("\\W(nagito|komaeda)\\W", "i").test(line)) return DR.ROLES.NAGITO;
		...
		if (RegExp("\\W(saionji|hiyoko)\\W", "i").test(line)) return DR.ROLES.HIYOKO;

		if (RegExp("\\W(sonia|nevermind)\\W", "i").test(line)) return DR.ROLES.SONIA;

		if (RegExp("\\W(sou?da|kazuichi)\\W", "i").test(line)) return DR.ROLES.KAZUICHI;
		...
		if (RegExp("\\W(juu?zou?|sakakura)\\W", "i").test(line)) return DR.ROLES.JUUZOU;

		if (RegExp("\\W(kyou?suke|munakata)\\W", "i").test(line)) return DR.ROLES.MUNAKATA;
	...
	function processRoleList(text) {
		var promises = [],
			...
			regex = /^(.*?)u\/([A-Za-z0-9-_]+)\s(.*)$/gim;

		var result, promise, line, username, character, offset, url, url_start, url_end;
